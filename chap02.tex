...
scheduled packets if their weights are scaled by the same factor. Therefore, we
can multiply the weight of each packet by $x \gg a$, which makes the additive
constant negligible.
...
be lower than $1$, this lemma holds trivially when $\frac{1}{\alpha} \leq
...
resulting in a gain of $2(\alpha + \delta)$.

The optimal algorithm can schedule all packets, resulting in a gain of $2 +
2(\alpha + \delta)$. The competitive ratio is:
...
paramteres $\alpha \geq 1$ and $\varepsilon > 0$.
\end{lemma}
...
\varepsilon$ for all paramteres $\alpha \geq 1$ and $\varepsilon > 0$.
\end{lemma}
...
Taking the limit of $\frac{W_{\EDF}}{\beta^{n-1}}$ as $n$ goes to $\infty$ gives us:
...
\frac{\beta^2}{\beta-1} + 2\beta + 2\beta^3
\end{equation}

...
the additive constant negligible. \xxx{TODO this works in general for any deterministic algorithm.}
...
= \frac{2\beta^2 - \beta}{\beta^2 - \beta + 1}
...
instance $I_2$, taking the most urgent packets. At step $n+1$,
...
The optimal algorithm schedules all packets except the first two packets of
weight $1$, resulting in a gain of:
...
\frac{2\beta^4 - 2\beta^3 + 3\beta^2 - 2\beta}{2\beta^4 - 2\beta^3 + 1} =
...
\begin{figure}[h]\centering
\input{tex/edf-instance-plot}
\caption{Upper bounds on the competitive ratio given by instances $I_1$, $I_2$, and $I_3$.}
\label{img:edf-instances-plot}
\end{figure}
...
\end{thm}
\begin{proof}
Given the parameter $\alpha$, the adversary can choose one of the three
instances from Lemmas \ref{lem:edf-I1}, \ref{lem:edf-I2}, and \ref{lem:edf-I3},
that maximizes the competitive ratio.

As shown in Figure \ref{img:edf-instances-plot}, this maximum is minimized when
the bounds for instances $I_1$ and $I_3$ are tight. Using basic algebra, we can
derive that the minimum is attained at $\alpha = \phi$, in which case the
competitive ratio equals $\phi$.
\end{proof}
